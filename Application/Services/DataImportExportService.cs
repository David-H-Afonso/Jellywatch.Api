...
public class DataImportExportService : IDataImportExportService
{
    ...
    public DataImportExportService(JellywatchDbContext context, ILogger<DataImportExportService> logger, IMetadataResolutionService metadata)
    {
        _context = context;
        _logger = logger;
        _metadata = metadata;
    ...
    public async Task<ServiceResult<ExportResult>> ExportAsync(int profileId)
    {
        var profile = await _context.Profiles.FindAsync(profileId);
        if (profile is null)
            return ServiceResult<ExportResult>.Fail("Profile not found", 404);
        ...
        var fileName = $"jellywatch-export-{profileId}-{DateTime.UtcNow:yyyyMMdd}.csv";
        return ServiceResult<ExportResult>.Ok(new ExportResult(bytes, "text/csv", fileName));
    }

    public async Task<ServiceResult<ImportPreviewDto>> ImportPreviewAsync(int profileId, Stream csvStream)
    {
        var rows = await ParseCsvAsync(csvStream);
        if (rows.Count == 0)
            return ServiceResult<ImportPreviewDto>.Fail("CSV is empty or has no valid rows", 400);
        ...
        return ServiceResult<ImportPreviewDto>.Ok(new ImportPreviewDto
        {
    ...
    public async Task<ServiceResult<ImportResultDto>> ImportAsync(int profileId, Stream csvStream, bool skipDuplicates, bool overwriteDates)
    {
        var rows = await ParseCsvAsync(csvStream);
        var imported = 0;
        var skipped = 0;
        ...
        return ServiceResult<ImportResultDto>.Ok(new ImportResultDto
        {
            Imported = imported,
            Skipped = skipped,
            Errors = errors.Take(50).ToList(),
