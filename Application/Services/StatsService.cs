...
public class StatsService : IStatsService
{
    ...
    public StatsService(JellywatchDbContext context)
    {
        _context = context;
    }

    public async Task<ServiceResult<WrappedDto>> GetWrappedAsync(int profileId, int? year)
    {
        ...
        {
            return ServiceResult<WrappedDto>.Ok(new WrappedDto { Year = targetYear });
        }
        ...
        return ServiceResult<WrappedDto>.Ok(new WrappedDto
        {
            Year = targetYear,
    ...
    public async Task<ServiceResult<List<CalendarDayDto>>> GetCalendarAsync(int profileId, int? year, int? month)
    {
        var targetYear = year ?? DateTime.UtcNow.Year;
        ...
        return ServiceResult<List<CalendarDayDto>>.Ok(grouped);
    }
    ...
    public async Task<ServiceResult<List<UpcomingEpisodeDto>>> GetUpcomingAsync(int profileId, int days, int? currentUserId)
    {
        ...
        if (watchedSeriesMediaItemIds.Count == 0)
            return ServiceResult<List<UpcomingEpisodeDto>>.Ok(new List<UpcomingEpisodeDto>());
